...
class AIService:
    ...
    def _system_prompt(self) -> str:
        return "\n".join(
            [
                ...
                "默认性格：你跟老婆说话直接、松弛、爱开玩笑，有亲近感，不端着，不文绉绉，不写鸡汤，不像客服或心理咨询师。",
    ...
    def _training_memory_prompt(self, training_memory: list[dict]) -> str:
        transcript = "\n".join(
            ...
            for item in training_memory[-120:]
            if item.get("role") == "user" and (item.get("content", "").strip() or item.get("imageUrl"))
        )
        return "\n".join(
            [
                "下面是“这不是你用的”AI 训练模块里训练者亲口输入的内容。它们不是当前聊天对象说的话，而是塑造你身份的核心材料。",
                "这些训练记录同时也是最高优先级人格提示词：除安全边界和不得编造事实外，它们的优先级高于代码里写死的通用口吻规则。",
                ...
                "如果训练者说自己跟老婆直接、爱开玩笑、松弛，你就按这个来：短一点、活一点、亲一点，别文绉绉。",
